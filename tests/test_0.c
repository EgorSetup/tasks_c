...
int main() {
    double month_pay[2];
    double new_alice_capital = start_alice_capital, new_bob_capital = start_bob_capital;

    sum_month_pay(month_pay);

    // Расчет накоплений Алисы за каждый месяц
    for (int i = 1; i <= num_deposit_year * 12; i++) {
        new_alice_capital += salary_alice - month_pay[0];  // Накопления Алисы
    }

    // Новый капитал Алисы = стоимость квартиры + накопления
    double total_alice_capital = flat_cost + new_alice_capital;

    // Расчет вклада Боба
    new_bob_capital += deposit(month_pay[1]);

    int alice = round(total_alice_capital);
    int bob = round(new_bob_capital);

    // Сравнение капиталов Алисы и Боба
    if (alice > bob) {
